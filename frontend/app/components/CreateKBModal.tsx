...
import { X, FolderOpen, Globe, Lock, Plus, Tag, Check } from "lucide-react";
...
interface CreateKBModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** When provided the modal edits that knowledge base instead of creating one. */
...
export default function CreateKBModal({
  ...
  const [createKnowledgeBase, { isLoading: isCreating }] =
    useCreateKnowledgeBaseMutation();
  const [updateKnowledgeBase, { isLoading: isUpdating }] =
    useUpdateKnowledgeBaseMutation();

  const isLoading = isCreating || isUpdating;

  // Seed the form from the KB being edited. Keyed on the id so reopening the
  // modal for a different KB refills the fields instead of keeping stale ones.
  useEffect(() => {
    if (!isOpen) return;

    setName(knowledgeBase?.name ?? "");
    setDescription(knowledgeBase?.description ?? "");
    setIsPublic(knowledgeBase?.visibility === "public");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await updateKnowledgeBase({ id: knowledgeBase.id, ...fields }).unwrap();
      } else {
        await createKnowledgeBase(fields).unwrap();
      }
      ...
      setError(
        (err as { data?: { detail?: string } })?.data?.detail ??
          `Could not ${isEditing ? "update" : "create"} this knowledge base. Please try again.`
      );
    }
  };
          ...
          <div className="flex items-start justify-between mb-7">
            <div>
              ...
              <h2 className="text-xl font-semibold text-foreground">
                {isEditing ? "Edit Knowledge Base" : "New Knowledge Base"}
              </h2>
              <p className="text-sm text-muted-foreground mt-0.5">
                {isEditing
                  ? "Update the name, description, tags or visibility."
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 flex items-center gap-1.5">
                ...
                Tags <span className="text-muted-foreground font-normal">(up to {MAX_TAGS})</span>
              </label>
              <div className={`flex flex-wrap gap-1.5 px-3 py-2.5 border rounded-xl bg-background min-h-[44px]
                               focus-within:ring-2 focus-within:ring-ring transition-shadow ${
                                 tags.length >= MAX_TAGS ? "border-border opacity-70" : "border-border"
                               }`}>
                {tags.map((tag) => (
                  <span
                    key={tag}
